		...
		try:	# SERIAL
			...
			if data != '':
				...
				if re.match(pattern, data):# rubbish..
					PAYLOAD+=(re.sub(pattern,'box6_\\1;temperature;\\2.\\3;'
						+ time.strftime("%Y%m%dT%H%M%SZ",time.gmtime()), data)
						+ re.sub(pattern,'box6_\\1;humidity;\\4.\\5;'
						+ time.strftime("%Y%m%dT%H%M%SZ",time.gmtime()), data)
						+ re.sub(pattern,'box6_\\1;light;\\6.\\7;'
						+ time.strftime("%Y%m%dT%H%M%SZ",time.gmtime()), data)
						+ re.sub(pattern,'box6_\\1;battery;\\8.\\9;'
						+ time.strftime("%Y%m%dT%H%M%SZ",time.gmtime()), data))
			s.close()
			print PAYLOAD
